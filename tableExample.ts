...
rows.map((row)=> m.TableRow({children: [{key: row.name}, m.TableCell({children: row.name})]}))
export function getTablePreview(): b.IBobrilChildren {
    return [m.Paper({style: { margin: 16, padding: 8 } }, m.Table({ 
        children: [
            m.TableHead({
                children : [
    ...
    m.Paper({style: { margin: 16, padding: 8 } }, [m.Table({
        children : [ m.TableHead ({
            children : [
                m.TableHeaderCell ({ children : "Dessert (100g serving)" }),
            ...
            rows.map((row)=> m.TableRow({children: [{key: row.name},
